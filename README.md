...
The dashboard stores aggregate usage rows only, under `~/Library/Application Support/CodexHub/usage-history-v1.json`. Rows are grouped by day, account email, and model for up to one year. CodexHub does not copy prompts, responses, command output, file contents, or full session logs into the dashboard store.
...
대시보드는 `~/Library/Application Support/CodexHub/usage-history-v1.json`에 집계 사용량 row만 저장합니다. row는 최대 1년 동안 날짜, 계정 이메일, 모델별로 묶입니다. CodexHub는 prompt, 응답, command output, 파일 내용, 전체 session 로그를 대시보드 저장소에 복사하지 않습니다.
...
대시보드 데이터는 `~/Library/Application Support/CodexHub/usage-history-v1.json`에 파생 집계값으로만 로컬 저장됩니다. 기본 보관 기간은 1년이며, 원본 Codex session 내용을 저장하지 않고 합계만 저장하므로 예상 용량은 보통 몇 MB 이하입니다. 설정 패널에서 이 대시보드 파일만 삭제할 수 있으며 Codex 인증 snapshot, 계정 연결 기록, 원본 `~/.codex/sessions` 로그는 삭제하지 않습니다.
